...
# -*- coding: utf-8 -*-
# run.py, parse_video/, support lieying python3 parse plugin port_version 0.4.0, based on pv.py command line
...
# call pv.py from command line
def _call_pv(args):
    # TODO support parse timeout
    py_bin = sys.executable
    now_dir = os.path.dirname(__file__)
    pv_bin = os.path.normpath(os.path.join(now_dir, './pv.py'))
    # run it
...
def _do_parse(url, hd_min=None, hd_max=None):
    # make pv.py command line arguments
    args = ['--fix-unicode']
    if hd_min != None:
        args += ['--min', str(hd_min)]
    if hd_max != None:
        args += ['--max', str(hd_max)]
    args += [url]
    # do call pv.py, NOTE just return json info
